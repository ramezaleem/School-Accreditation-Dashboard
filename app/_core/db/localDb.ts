...
import type { Domain, Indicator, Staff, Evidence, Admin } from '@/app/_core/types';
...
const ADMIN_PASSWORDS: Record<string, string> = {
  admin: btoa('admin123'), // password: admin123
};
...
export const localDb = {
  ...
  checkAdminLogin(username: string, password: string): Admin | null {
    ...
    const expected = ADMIN_PASSWORDS[username];
    if (!expected || btoa(password) !== expected) return null;
    return admin;
  },
  ...
  getStats() {
    return {
      ...
      indicators: this.getIndicators().length,
      evidences:  this.getEvidences().length,
      staff:      this.getStaff().filter(s => s.is_active).length,
    };
  },
  getDomainsWithStats() {
    return this.getDomains().map(d => {
      const inds    = this.getIndicatorsByDomain(d.id);
      const evCount = inds.reduce((sum, ind) => sum + this.getEvidencesCountByIndicator(ind.id), 0);
      return { ...d, indicators_count: inds.length, evidences_count: evCount };
    });
  },
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        ...
        try {
          ...
          resolve({ url: '', name: file.name }); // localStorage full
        }
      };
  ...
  getFileUrl(url: string): string {
    ...
    return localStorage.getItem(url.replace('local://', '')) ?? '';
  },
